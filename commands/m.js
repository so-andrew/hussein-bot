...
module.exports = {
    ...
    async execute(message, args){
        ...
        let macroDB;
        if(!message.guild) macroDB = message.client.macros.get("231179508114391041");
        else macroDB = message.client.macros.get(message.guild.id);
        if(args[0] === "create"){
            //at least 3 arguments, args[1] cannot be a URL, args[2] must be a URL
            if(args.length === 3 && !urlRegex.test(args[1]) && urlRegex.test(args[2])) createMacro(message, args, macroDB);
            else return message.channel.send("Command usage: `!m create [name] [link]`");
        }
        else if(args[0] === 'delete'){
            if(args.length === 2) deleteMacro(message, args[1], macroDB);
            else return message.channel.send("Command usage: `!m delete [name]`");
        }
        else if(args[0] === 'info'){
            if(args.length === 2) macroInfo(message, args[1], macroDB);
            else return message.channel.send("Command usage: `!m info [name]`");
        }
        else if(args[0] === 'edit'){
            if(args.length === 3) editMacro(message, args, macroDB);
            else return message.channel.send("Command usage: `!m edit [name] [name/link]`");
        }
        else if(args[0] === 'search'){
            if(args.length === 2) searchMacro(message, args[1], macroDB);
            else return message.channel.send("Command usage: `!m search [name]`");
        }
        else if(args[0] === 'list') listMacros(message, macroDB);
        else retrieveMacro(message, args[0], macroDB);
    }
}

async function createMacro(message, args, macroDB){
    console.log(`Command m create received from ${message.author.username} with arguments ${args}.`)
    try{
        const macro = await macroDB.create({
...
async function retrieveMacro(message, arg, macroDB){
...
async function deleteMacro(message, arg, macroDB){
...
async function macroInfo(message, arg, macroDB){
...
async function editMacro(message, args, macroDB){
    ...
    if(macro){
        if(message.author.id === macro.get('creatorID') || message.author.id === config.ownerID){
            if(urlRegex.test(args[2])){
                ...
                const newMacro = await macroDB.create({
                    name: args[2].toLowerCase(),
                    text: macro.get('text'),
                    creatorID: macro.get('creatorID'),
                    creatorName: macro.get('creatorName'),
                });
                await macroDB.destroy({ where: { name: args[1].toLowerCase() } });
...
async function listMacros(message, macroDB){
    console.log(`Command m list received from ${message.author.username}.`);
    const macroList = await macroDB.findAll({ attributes: ['name'] });
...
async function searchMacro(message, arg, macroDB){
  ...
  const queryResults = await macroDB.findAll({ where:
